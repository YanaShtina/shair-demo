...
/* harmony default export */ var scrollTo = ({
  scroll: function scroll() {
    ...
    var header = document.querySelector('.header');
    var headertHeight = header.offsetHeight;

    // Функция, которая проверяет, что элемент хотя бы частично виден на экране
    var isElementPartiallyInViewport = function isElementPartiallyInViewport(el) {
      var rect = el.getBoundingClientRect();
      var windowHeight = window.innerHeight || document.documentElement.clientHeight;
      return rect.bottom >= 0 && rect.top <= windowHeight;
    };

    /* // Функция обработчика события прокрутки
    const handleScroll = () => {
     
    
      if (isElementPartiallyInViewport(header)) {
        console.log('Элемент хотя бы частично виден на экране!');
    ...
    // Вызов функции handleScroll для проверки видимости элемента при загрузке страницы
    handleScroll(); */
  }
});
  ...
  init: function init() {
    ...
    openPopupButtons.forEach(function (button) {
      button.addEventListener('click', function (e) {
        ...
        var btn = popupBg.querySelector('.popup__btn');
        ...
        btn.addEventListener('click', function () {
          ...
          popup_enableBodyScroll(targetElement);
          var topOffset = 50;
          var scrollTarget = document.querySelector(".contact");
          var elementPosition = scrollTarget.getBoundingClientRect().top;
          var offsetPosition = elementPosition - topOffset;
          window.scrollBy({
            top: offsetPosition,
...
;// CONCATENATED MODULE: ./src/modules/select.js
/* harmony default export */ var modules_select = ({
  init: function init() {
    document.addEventListener("DOMContentLoaded", function () {
      ...
      var optionItems = document.querySelectorAll('.select-options div');
      optionItems.forEach(function (option) {
        option.addEventListener('click', function () {
          selectStyled.textContent = option.textContent;
          selectStyled.classList.add('selected');
...
/* harmony default export */ var slider = ({
  init: function init() {
    var swiper = new core('.swiper.swiper-main', {
    ...
    var swiperD = new core('.swiper.swiper-d', {
      modules: [Navigation, Pagination, Autoplay, EffectFade],
      speed: 900,
      pagination: {
        el: '.swiper-pagination',
  ...
  init: function init() {
    ...
    burger.addEventListener('click', function () {
      if (burger.classList.contains('active')) {
        burger.classList.remove('active');
        body.classList.remove('active');
        burger_enableBodyScroll(nav);
      } else {
        burger.classList.add('active');
        body.classList.add('active');
        burger_disableBodyScroll(nav);
      }
...
/* harmony default export */ var modules_form = ({
  init: function init() {
    var form = document.querySelector('.form');
    var regexp = /^\d{1,15}$/g;
    var tel = document.querySelector('.input-tel');
    ...
    tel.addEventListener('input', function () {
      ...
      this.value = cleanedValue.slice(0, 20);
    });
    function isFieldNotEmpty(fieldValue) {
      return fieldValue.trim() !== '';
    }
    function formSended(_x) {
      return _formSended.apply(this, arguments);
    }
    function _formSended() {
      _formSended = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(e) {
        var checkbox, parent, parent2, parent3, complited, formData;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              checkbox = form.querySelector('.checkbox');
              e.preventDefault();
              parent = tel.closest('.wrap');
              parent2 = name.closest('.wrap');
              parent3 = select.closest('.wrap');
              console.log('parent3', parent3);
              complited = true;
              if (!isFieldNotEmpty(tel.value)) {
                parent.classList.add('error');
                parent.querySelector('.error-span').textContent = 'заполните поле';
              } else {
                parent.classList.remove('error');
              }
              if (!isFieldNotEmpty(name.value)) {
                parent2.classList.add('error');
                parent2.querySelector('.error-span').textContent = 'заполните поле';
              } else {
                parent2.classList.remove('error');
              }
              if (!select.classList.contains('selected')) {
                parent3.classList.add('error');
                parent3.querySelector('.error-span').textContent = 'выберите вид консультации';
              } else {
